## [2026-08-09] schema | Add problems tracking

Added `wiki/problems/` to the schema for logging solved LeetCode/interview problems: per-problem pages (`type: problem`, difficulty + pattern frontmatter, optimal-approach + complexity + gotcha body) plus `wiki/problems/index.md` as a single scannable table. Updated AGENTS.md with the new directory, frontmatter type, and a "Log a problem" operation. Linked from wiki/index.md. No problems logged yet — structure only.

## [2026-07-10] ingest | Data Modeling for System Design Interviews

Ingested Hello Interview's data modeling core-concepts guide via URL (staged to raw/2026-07-10-data-modeling-hellointerview.md). Created 1 source summary. This wiki's existing reference coverage of every database type and schema mechanic the article touches (Relational Model, NoSQL, Document Database, Key-Value Store, Wide-Column Store, Graph Database, Primary/Foreign Key, Database Index, Data Denormalization, Database Sharding) already exceeds the article's depth, so created 1 new concept page for the genuinely new piece: Data Modeling, capturing the article's interview-specific methodology (three-factor schema design driver — data volume, access patterns, consistency requirements — plus a default-to-SQL selection framework and a 6-step interview checklist) as a cross-linking hub over the existing detailed pages rather than duplicating them. Updated Graph Database with an Interview Caution section (Facebook/MySQL, LinkedIn/Twitter SQL examples countering the graph-DB-for-social-networks intuition), NoSQL with an Interview Default section, and Foreign Key with a Cost at Scale section (dropping FK constraints for write throughput at large scale). Updated Hello Interview entity with the fifth source.

## [2026-07-10] ingest | Sharding in System Design Interviews

Ingested Hello Interview's sharding core-concepts guide via URL (staged to raw/2026-07-10-sharding-hellointerview.md). Created 1 source summary. This wiki's existing sharding coverage (from the 2026-05-11 AlgoMaster ingest: Database Sharding, Shard Key, Hash/Range/Directory-Based Sharding, Cross-Shard Query, Data Rebalancing) already exceeded the article's depth on strategy mechanics, so focused new work on genuine gaps: created 1 new concept page (Saga Pattern — cross-shard/cross-service transaction consistency via compensating transactions, not previously documented anywhere despite Two-Phase Commit already existing as an entity). Updated Database Sharding with a Partitioning vs. Sharding terminology section, a Cross-Shard Transactions section (2PC vs. sagas vs. avoid-by-design, distinct from the existing read-side Cross-Shard Query page), a Modern Database Solutions comparison table (added Vitess and Citus, not mentioned anywhere in the wiki before), and an Interview Approach section. Updated Shard Key with the "celebrity problem" naming and dynamic-shard-splitting mitigation. Updated Partitioning with the same terminology cross-link. Updated Hello Interview entity with the fourth source.

## [2026-07-10] ingest | Caching for System Design Interviews

Ingested Hello Interview's caching core-concepts guide via URL (staged to raw/2026-07-10-caching-hellointerview.md). Created 1 source summary. This wiki's existing caching coverage (from the AWS/system-design-primer/HTTP-caching/cache-invalidation-strategies ingests) already exceeded the article's depth on cache-aside/write-through/write-behind/read-through, LRU/LFU/FIFO eviction, and cache stampede, so most of the article needed no new pages. Created 2 new concept pages for genuine gaps: In-Process Caching (local per-server memory, distinct from both Application Caching's ownership axis and In-Memory Cache's external-store framing) and Cache Hot Key (mirrors the existing Rate Limiter Hot Key page but for the caching domain, with replication/local-fallback/rate-limit mitigations). Updated Caching with a client-side caching clarification, hot-key cross-link, and an Interview Approach section. Updated Hello Interview entity with the third source.

## [2026-07-10] ingest | API Design for System Design Interviews

Ingested Hello Interview's API design core-concepts guide via URL (staged to raw/2026-07-10-api-design-hellointerview.md). Created 1 source summary. Found most protocol-level ground (REST, GraphQL, gRPC, rate limiting, over/under-fetching) already well covered by existing pages, so focused new pages on genuinely uncovered material: Cursor-Based Pagination (previously only a one-line mention on Page Pagination), API Versioning (URL vs. header strategies), Authentication vs. Authorization (the core distinction, previously undocumented despite being assumed by JWT/RBAC references elsewhere), API Key and JWT (the two credential mechanisms), and RBAC. Updated Page Pagination with a proper link to the new Cursor-Based Pagination page instead of a bare mention.

## [2026-07-08] ingest | Lecture 12. Leave a Clean Handoff at the End of Every Session

Ingested Learn Harness Engineering's twelfth lecture on ending sessions in a verifiably clean state. Created 1 source summary, 3 new concept pages (Quality Document, Cleanup Loop, Harness Simplification — the series' first argument for periodically removing harness structure rather than adding it). Folded the five-condition clean-state definition and Lehman's-laws citation directly into Harness Initialization Flow's clock-out description, and the session-integrity/atomicity framing into Agent State ACID Principles, rather than creating duplicate concepts. Updated Capability Gap with the twelfth case study (12-week longitudinal Electron project, 68%/61%/60+min without cleanup vs 97%/95%/9min with) and Learn Harness Engineering with the lecture-12 summary.

## [2026-07-08] ingest | Lecture 11. Making the Agent's Runtime Observable

Ingested Learn Harness Engineering's eleventh lecture on observability as an architectural property. Created 1 source summary, 4 new concept pages (Layered Observability, Task Trace — citing Google's Dapper paper, Sprint Contract, Evaluator Rubric). Extended Harness Engineering's Feedback Subsystem with the runtime/process observability split, and Worker/Checker Separation with the missing reliability mechanism (rubrics) it lacked. Recognized the lecture's Anthropic DAW experiment (March 2026) as a distinct case study from the game-editor experiment already documented from Lecture 01/09, so added it as new rather than conflating them — updated Anthropic entity and Capability Gap (eleventh case study) accordingly. Updated Learn Harness Engineering with the lecture-11 summary.
